...
class ImageProcessorApp:
    ...
    def show_histogram(self):
        ...
        if len(img.shape) == 2:
            channels, colours, labels = [img], [(200,200,200)], ["Gray"]
        else:
            channels = [img[:,:,i] for i in range(3)]
            colours  = [(255,60,60),(60,255,60),(60,60,255)]
            labels   = ["Blue","Green","Red"]
 
        # Real pixel counts — find global max for shared Y scale
        hists = [cv2.calcHist([ch],[0],None,[256],[0,256]) for ch in channels]
        global_max = max(h.max() for h in hists) or 1
 
        for hist, colour in zip(hists, colours):
            pts = [[int(x*hist_w/256),
                    hist_h - int((hist[x,0]/global_max)*(hist_h-40)) - 1]
                   for x in range(256)]
            pts = np.array([[0,hist_h-1]] + pts + [[hist_w-1,hist_h-1]], dtype=np.int32)
            cv2.fillPoly(canvas, [pts], colour)
 
        blended = cv2.addWeighted(canvas, 0.75, np.zeros_like(canvas), 0.25, 0)
 
        # Labels
        for x in range(0, 256, 64):
            cv2.putText(blended, str(x), (int(x*hist_w/256), hist_h-5),
        ...
        for i, (lbl, col) in enumerate(zip(labels, colours)):
            cv2.putText(blended, f"  {lbl}", (10, 40+i*18),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.4, col, 1)
 
        self._display_result(blended, update_current=False)
        self._status("Colour Histogram displayed — channels: " + ", ".join(labels))
